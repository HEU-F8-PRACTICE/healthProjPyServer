...
def solveData(data, pool, sock):
    for dataStr in str(data, encoding='utf-8').split(';'):
        ...
        if dataList[0] == 'facility':
            ...
            t.start()
        if dataList[0] == 'getPatientInfo':
            sql = "select * from patient where patientid in (select patientid from facipati where facilityid=" + dataList[1] + ");"
            infoList = pool.execute(sql, return_one=True)
            info = str(infoList[0]) + ',' + str(infoList[1]) + ',' + str(infoList[2]) + ',' + str(infoList[3]) + ',' + str(infoList[4])
            try:
                sock.send(bytes(info, encoding='utf-8'))
            except:
                pass
